...
![image](https://gyazo.com/5ec843da68eb06f4d8a05cc73a98a303.png)
...
ここでは単一ソースでの画像合成をベースに複数ソースでの画像の合成を行う．別々のソースの補間や画像の調和，モーフィングなどの様々なタスクに対してほぼ同じフレームワークで解決することができる．
#### 複数画像の空間的な補間
空間的な補間は画像のインペインディングや，調和，パノラマ作成など様々なアプリケーションに応用される．
タスクは，レンガと皮の画像の自然な合成など複数画像の空間的な補間である．単一画像の目的関数をそのまま適用すると，それぞれのソースを滑らかに補間することができない．境目の局所領域だけ自然に補間されてしまい全体としては不自然になる（fig8(C））
![image](https://i.gyazo.com/ab098be274d985a5c3ef98cb5421a815.png)
そこで複数ソースの合成では，新たにそれぞれの画像間を滑らかに遷移するパラメータαを導入し，全体的に滑らかに補間できるように工夫されている．パラメータαは0から１までの値をとる（ex:画像の左が０右が１）．異なるテクスチャの補間を行う際，目的関数は

![muluti_objective_func](https://latex.codecogs.com/gif.latex?E_%7Bbrend%7D%28T%2C%5C%7BS_1%2CS_2%5C%7D%29%3D%20%5Calpha%20E%28T%2CS_1%29&plus;%281-%5Calpha%20%29E%28T%2CS_2%29)

となる．ここで![S1](https://latex.codecogs.com/gif.latex?S_1)，![S2](https://latex.codecogs.com/gif.latex?S_2)は合成元となるそれぞれの画像，Tはテクスチャ間の補完したいターゲット領域，Eは単一画像のものと同じ関数である．複数画像の補間の全体の流れは，　
1. それぞれのソースで別々にターゲット領域の画素と勾配を単一画像同様に探索と投票を用いて求める．
2. ソースごとに求めたターゲット領域の輝度の情報と勾配の情報を合成し，新たにターゲット領域の輝度と勾配を得る
3. ２で得た輝度と勾配の情報を用いてポアソン方程式を解き，ターゲット領域の画像を求める．

２のターゲット領域の勾配の情報を合成する際に，従来はL2ノルムが用いられていたが，L0ノルムに置き換えることでよりボケの少ない結果を得ることができている（fig8(b））
### 複数画像の時間的な補間
二つの画像間を自然に繋ぐようなフレーム画像を生成し，自然に遷移するような動画を作成するタスク（モーフィング）．
時間的に近いフレームとの差をみるように今までのをベースに新しく目的関数を設定する．目的関数は
