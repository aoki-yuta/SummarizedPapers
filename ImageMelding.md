...
論文ではまずシンプルな[A_単一ソースの画像合成](#A_単一ソースの画像合成)について導入し，それを応用して[B_複数ソースの画像合成と応用アプリケーション](#B_複数ソースの画像合成と応用アプリケーション)を導入している
### A_単一ソースの画像合成
...
### B_複数ソースの画像合成と応用アプリケーション

## この論文で何が出来るようになった？and/or 何が良くなった？　
従来の手法に比べ，より幾何学的な不自然さや色合いの不自然さを減らし，画像の特徴を捉えてた画像合成手法である．．これを応用することでHarmonizeやモーフィングなど画像の合成の様々なアプリケーションへと応用可能
### 画像インペインディング
パッチマッチにリフレクションやnon uniform scaleを加えることでより複雑な構造を捉えることができるようになり，color gainやcolor bias，勾配項によって色の不自然さを減らすことが可能となった．
...
## 実装はある？
Matlab（中に.mexw64ファイルが含まれるためwindowsでしか動かない）
[code](https://www.ece.ucsb.edu/~psen/melding)
